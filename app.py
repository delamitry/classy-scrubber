...
def mask_entities_and_contacts(model_name, text, file_path, dynamic_mask, entity_type_selection):
    ...
    if num_emails > 0:
        feedback_messages.append(f"Masked {num_emails} email address(es) with regex.")
    if num_ssns > 0:
        feedback_messages.append(f"Masked {num_ssns} SSN(s) with regex.")
    if num_phones > 0:
        feedback_messages.append(f"Masked {num_phones} phone number(s) with regex.")
    if num_cards > 0:
        feedback_messages.append(f"Masked {num_cards} credit card number(s) with regex.")
    
    # General feedback if nothing was masked
    if not any([masked_entities, num_emails, num_ssns, num_phones, num_cards]):
        feedback_messages.append("No sensitive information was found to mask with regex.")
